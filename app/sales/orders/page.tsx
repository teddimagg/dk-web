...
export default function OrdersPage() {
  ...
  function open(e: React.FormEvent) {
    ...
    if (!id) return setLookupError(t("sales.orders.enterNumber"));
    setLookupError(null);
    remember(id);
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <Card className="p-6">
        <CardTitle icon={<Search />} className="mb-4">
          {t("sales.orders.openTitle")}
        </CardTitle>
        <p className="mb-5 text-sm leading-relaxed text-fog">{t("sales.orders.openInfo")}</p>
        <form onSubmit={open} className="flex items-end gap-2">
          <Field label={t("sales.orders.numberLabel")} className="flex-1" error={lookupError ?? undefined}>
            <Input
              value={lookup}
              onChange={(e) => setLookup(e.target.value)}
              placeholder={t("sales.orders.numberPlaceholder")}
              autoFocus
            />
          </Field>
      ...
      <Card className="p-6">
        <CardTitle icon={<ClipboardList />} className="mb-4">
          {t("sales.orders.createTitle")}
        </CardTitle>
        <p className="mb-5 text-sm leading-relaxed text-fog">{t("sales.orders.createInfo")}</p>
        <Button onClick={() => setCreateOpen(true)}>
          <Plus className="size-4" /> {t("sales.orders.new")}
        </Button>
      </Card>
