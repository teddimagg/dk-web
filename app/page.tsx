  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid items-end gap-6 lg:grid-cols-[1fr_400px]">
        <div>
          <PageHeader eyebrow={`Data based on ${company?.name ?? "…"}`} title="Overview Panel" />
          <div className="mt-4 flex flex-wrap items-center gap-2 text-[13px] text-fog">
            ...
            <span className="rounded-full border border-line bg-white px-3 py-1 tnum">
              {inv ? `${formatInt(inv.length)} invoices · ${periodLabel(period)}` : "…"}
            </span>
            <span className="rounded-full border border-line bg-white px-3 py-1">
              Updated {invoices.dataUpdatedAt ? timeAgo(new Date(invoices.dataUpdatedAt).toISOString()) : "…"}
            </span>
            <button
          ...
          {truncated && (
            <p className="mt-2 text-xs text-[#9a6a10]">
              This range exceeds 40.000 invoices — stats reflect the first 40.000 fetched. Narrow
          ...
          <div className="glass-green relative h-full rounded-card p-5 shadow-card">
            <div className="flex items-center justify-between">
              <span className="flex items-center gap-2 text-[13px] font-semibold text-ink">
                <Zap className="size-4" /> Account Insights
              </span>
            ...
            {m ? (
              <>
                <p className="mt-3 text-[1.35rem] font-medium leading-snug text-ink">
                  <strong>{m.monthName}</strong> closed at{" "}
                  <strong className="tnum">{formatCompact(m.monthTotal)} ISK</strong> across{" "}
                  <strong className="tnum">{m.monthCount}</strong> invoices
                </p>
                {m.monthDelta != null && (
                  <p className="mt-2 text-[13px] font-medium text-ink/70 tnum">
                    {m.monthDelta >= 0 ? "▲" : "▼"} {formatPercent(Math.abs(m.monthDelta))} vs the month before
                  </p>
                )}
              </>
            ) : inv && inv.length === 0 ? (
              <p className="mt-3 text-[1.35rem] font-medium leading-snug text-ink/70">
                No invoicing activity in <strong className="text-ink">{periodLabel(period)}</strong>
              </p>
            ) : invoices.error ? (
              <p className="mt-3 text-[1.35rem] font-medium leading-snug text-ink/70">
                Couldn't reach dkPlus — check the token or try refreshing
              </p>
            ) : (
      ...
      {inv && inv.length === 0 && (
        <Card className="flex flex-col items-center gap-3 p-12 text-center">
          ...
          <div>
            <p className="font-medium text-ink">No invoices in {periodLabel(period)}</p>
            <p className="mx-auto mt-1 max-w-sm text-sm text-fog">
              This company has no invoice activity in the selected period — pick another year or
              browse everything.
            </p>
          </div>
          <button
        ...
        {loading || !m ? (
          ...
          <>
            <StatCard
              ...
              unit="%"
              left={{ value: formatInt(m.settled.length), label: "Settled" }}
              right={{ value: formatInt(m.open.length), label: "Open" }}
              meter={m.settledPct}
            />

            <Card className="flex flex-col gap-2 p-5">
              <CardTitle icon={<Activity />}>Collection Rate</CardTitle>
            ...
            <StatCard
              ...
              unit="%"
              left={{ value: formatInt(m.overdue.length), label: "Overdue" }}
              right={{ value: formatCompact(m.overdueAbs), label: "ISK at risk" }}
      ...
      <section className={`grid gap-4 lg:grid-cols-3 ${invoices.error ? "hidden" : ""}`}>
          ...
          <Card className="flex flex-col gap-4 p-5">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="text-center">
                ...
                <p className="mt-1 text-xs text-fog">Average Account Stats</p>
              </div>
              <div className="space-y-3 text-right text-[13px]">
                <p>
                  ...
                  <span className="text-fog">Grouped clients</span>
                </p>
                <p>
                  <span className="stat-numeral block text-lg tnum">{formatInt(cust.blocked)}</span>
                  <span className="text-fog">Blocked accounts</span>
                </p>
              </div>
            </div>
        ...
        {inv && inv.length === 0 ? (
          <Card className="grid place-items-center p-5 text-sm text-fog">
            No invoice data in {periodLabel(period)}
          </Card>
        ) : m ? (
          <Card className="flex flex-col gap-3 p-5">
            <CardTitle icon={<Timer />}>Timely Closures</CardTitle>
            ...
            <p className="text-right text-[11px] text-mist">daily invoiced volume · last 42 days of data</p>
          </Card>
        ) : (
          <Card className="p-5">
            <StatSkeleton />
          ...
          <div className="space-y-3 text-[13px]">
            {m?.latestInvoice && (
              <div className="flex items-start gap-2.5">
                ...
                <p className="leading-snug">
                  Latest invoice <strong>#{m.latestInvoice.Number}</strong> —{" "}
            ...
            {m?.biggest && (
              <div className="flex items-start gap-2.5">
                ...
                <p className="leading-snug">
                  Largest recent invoice <strong>#{m.biggest.Number}</strong> at{" "}
            ...
            {clockedIn.data && (
              <div className="flex items-start gap-2.5">
                ...
                <p className="leading-snug">
                  ...
                  {clockedIn.data.length === 1 ? "employee is" : "employees are"} clocked in right now
            ...
            {cust?.latestModified && (
              <div className="flex items-start gap-2.5">
                ...
                <p className="leading-snug">
                  Customer <strong>{cust.latestModified.Name}</strong> updated
      ...
      <section>
        <h2 className="mb-3 text-sm font-medium text-fog">Modules</h2>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
          {MODULES.map((mod) => (
            ...
            >
              ...
              <span className="grid size-9 place-items-center rounded-xl bg-ink text-white [&>svg]:size-4.5">
                <mod.icon />
              </span>
              <p className="mt-3 text-sm font-medium text-ink">{mod.label}</p>
